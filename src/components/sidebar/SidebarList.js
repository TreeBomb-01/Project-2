// components/sidebar/SidebarList.js
...
function SideBarList({ $target, initalState, onEditing }) {
    ...
    this.createTreeView = (data) => {
        ...
        for (const key in data) {
            if (data[key].documents && data[key].documents.length > 0) {
                str += `
        ...
        $list.innerHTML = `
            <ul class="documentList">
                ${this.state
                    .map(
                        (document) =>
                            `<li class="dataList" data-id="${document.id}">📄 ${document.title}
                                <button class="addBtn" data-id="${document.id}">➕</button>
